...
class AbstractPDMPlanner(AbstractPlanner, ABC):
    ...
    def _get_discrete_centerline(
        ...
        for lane in route_plan:
            centerline_discrete_path.extend(lane.baseline_path.discrete_path) # append加一个，extend加一个列表
        ...
        for lane_id in intersecting_lanes:
            if lane_id in self._route_lane_dict.keys():
                ...
                lane_state_se2_array = np.array(
                    [state.array for state in lane_discrete_path], dtype=np.float64 # 将StateSE2转换成numpy.array
                )
                # calculate nearest state on baseline
                # ego_position_array[None, ...] 通过在数组最前面增加一个维度，改变其形状
                # 为了在后续的广播操作（broadcasting）中匹配 lane_state_se2_array 的维度
                lane_distances = (
                    ego_position_array[None, ...] - lane_state_se2_array
                ) ** 2
                lane_distances = lane_distances.sum(axis=-1) ** 0.5 #表示在最后一个轴（axis=-1）上对坐标差的平方值求和

                # calculate heading error
                heading_error = (
                    lane_discrete_path[np.argmin(lane_distances)].heading - ego_heading
                )
                heading_error = np.abs(normalize_angle(heading_error)) # 规范化到正pi和负pi之间
