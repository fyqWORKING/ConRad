...
class BatchIDMPolicy:
    ...
    def __init__(
        self,
        fallback_target_velocity: Union[List[float], float], # 每个参数可以是单个 float 值，或是包含多个 float 值的 List，以支持批量策略。
    ...
    def propagate(
        self,
        previous_idm_states : npt.NDArray[np.float64], #包含之前时刻的车辆状态，包括位置和速度
        leading_agent_states: npt.NDArray[np.float64], #包含前车的信息，包括前车的位置、速度和车长
        longitudinal_idcs   : List[int],               #一个列表，表示要在这次计算中使用哪些车辆的策略（即哪些车辆参与计算）
        sampling_time       : float,                   #表示时间步长，即从当前状态推进到下一个状态所用的时间
        ...
        # state variables
        x_agent, v_agent = (
            previous_idm_states[:, StateIDMIndex.PROGRESS],    #车辆的当前位置
            previous_idm_states[:, StateIDMIndex.VELOCITY],    #车辆的当前速度
        )

        x_lead, v_lead, l_r_lead = (
            leading_agent_states[:, LeadingAgentIndex.PROGRESS],   #前车的位置
            leading_agent_states[:, LeadingAgentIndex.VELOCITY],   #前车的速度
            leading_agent_states[:, LeadingAgentIndex.LENGTH_REAR],#前车的长度
        )

        # parameters
        target_velocity, min_gap_to_lead_agent, headway_time, accel_max, decel_max = (
            self._target_velocities[longitudinal_idcs],         #目标速度（期望速度）
            self._min_gap_to_lead_agent[longitudinal_idcs],     #与前车的最小安全距离
            self._headway_time[longitudinal_idcs],              #跟车时间（反映车辆与前车之间的安全时间间隔）。
