...
class SimulationRunner(AbstractRunner):
    ...
    def run(self) -> RunnerReport:
        ...
        del self.planner._model_loader # 避免显存爆炸 
        # # 将运行时间保存到CSV文件
        # with open('/home/fyq/Downloads/execution_times.csv', 'w', newline='') as file:
        #     writer = csv.writer(file)
        #     writer.writerow(['Run Number', 'Execution Time (seconds)'])  # 写入表头
        #     writer.writerows(self.planner._execution_times)  # 写入每次的运行时间
        # print("运行时间已保存到 'execution_times.csv' 文件中。")
        # with open('/home/fyq/Downloads/planner.csv', 'w', newline='') as file:
        #     writer = csv.writer(file)
        #     writer.writerow(['Run Number', 'planner'])  # 写入表头
        #     writer.writerows(self.planner._planner_types)  # 写入每次的运行时间
        # print("运行时间已保存到 'planner.csv' 文件中。")
